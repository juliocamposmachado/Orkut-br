...
if (!supabaseUrl || !supabaseServiceKey || 
  ...
  console.warn('Supabase não configurado para create-epic-community API')
}
...
export async function POST(request: NextRequest) {
  try {
    // Se Supabase não estiver configurado, retornar resposta de demonstração
    if (!supabase) {
      return NextResponse.json({
        success: true,
        message: 'Comunidade épica criada (modo demo)!',
        community: {
          id: Date.now(),
